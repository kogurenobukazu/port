...
//        cLogic.setCustomerBeanFromRequestToSession(request);
        String result = cLogic.update(customer);

        if(result.equals("1")) {
            filePath = "/WEB-INF/customer/update_success.jsp";
        }else if(result.equals("0")) {
            filePath = "/WEB-INF/customer/update_fail.jsp";
        }
    ...
    private void procAdd(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        CustomerBean customer = (CustomerBean) session.getAttribute("customer");
        CustomerLogic cLogic = new CustomerLogic();
        String result = cLogic.add(customer);
    ...
    private void procDelete(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        CustomerBean customer = (CustomerBean) session.getAttribute("customer");
        CustomerLogic cLogic = new CustomerLogic();
        String result = cLogic.delete(customer);
     ...
     * @param response  HTTPのレスポンス
     */
    private void procEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException {

        RequestDispatcher Dispatch = request.getRequestDispatcher("/WEB-INF/customer/edit.jsp");
    ...
    private void procNewConfirm(HttpServletRequest request, HttpServletResponse response) throws ServletException,
        ...
        customer.setCustomerBeanFromRequestToSession(request);

        /*RequestDispatcher  = request.getRequestDispatcher("/WEB-INF/customer/new_confirm.jsp");*/
        RequestDispatcher Dispatch = request.getRequestDispatcher("/WEB-INF/customer/new_confirm.jsp");
        Dispatch.forward(request, response);
